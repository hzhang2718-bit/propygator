...
# edge exists only at call time, when the user asks for a live view. Per features.md
# §1.4 the animation loop is display-only and deliberately not snapshot-tested; the
# panels reuse the shipped 1.1 ``_draw_*`` primitives via their chunk-6
# default-preserving seams plus the new ``_draw_sky_track``.
...
def _plot_sky_glyph(ax: Axes, azel: AzElRange, style: dict[str, Any]) -> None:
    """Scatter one body/satellite glyph on the polar sky axis.

    The polar mapping shared with ``_draw_sky_track``: theta = azimuth, radius = zenith
    angle (90° − elevation). A below-horizon body (radius > 90°) lands outside the
    ``rlim`` and is clipped — exactly the desired "not currently up" behaviour.
    """
    ax.scatter(math.radians(azel.azimuth_deg), 90.0 - azel.elevation_deg, **style)
...
def live_track(
    ...
    *,
    ...
    chunk-6 ``_draw_*`` seams, so no tick re-crosses the JVM in an O(N) loop).
    ...
    *default* ``%matplotlib inline`` backend draws only a static first frame (documented
    ...
    from ..plotting.timeseries import _HOURS_XLABEL, _draw_altitude, _draw_speed
    ...
    def update(_frame: object) -> list[Artist]:
        ...
        now = Epoch.now()
        engine.maybe_rebuild(now)
        buffer = engine.buffer

        state_now = engine.state_at(now)  # .epoch is clamped to the realized edge
        clamped = state_now.epoch
        # One ITRF conversion per frame, reused for the sub-point *and* the ground
        # speed; the inertial speed reads straight off the TEME state (|v| invariant).
        ...
        with _mpl_style():
            # Ground track: the static symmetric buffer (end triangle suppressed — in a
            # centred buffer the "end" is the *future* leading edge, not the satellite),
            # plus the live heading-marked sub-satellite point and (4-panel) the
            # station. The legend gains a "current position" key, and a "ground station"
            # key when a station is present (forwarded as proxies into the primitive).
            ax_ground.clear()
            ground_handles = [
            ...
            # _track_heading_deg clamps the raw sample index onto its segment.
            heading = _track_heading_deg(
                lon, lat, at_index=int(round(elapsed_s / output_step))
            )
            live_marker = _heading_marker(heading)
            ax_ground.scatter(
                geo.longitude_deg, geo.latitude_deg, marker=live_marker, **_LIVE_MARKER
            )

            # Altitude: the static curve + a moving now-cursor.
            ax_alt.clear()
            _draw_altitude(ax_alt, buffer)
            ax_alt.axvline(elapsed_h, **_CURSOR_STYLE)

            # Speed: inertial + ground-relative overlaid (their gap is the
            # Earth-rotation story), both from the engine's precomputed arrays (no
            # per-frame to_frame), plus the now-cursor; the bottom panel owns the x-axis
            # label.
            ax_speed.clear()
            _draw_speed(
                ax_speed,
            ...
            ax_speed.axvline(elapsed_h, **_CURSOR_STYLE)
            ax_speed.set_xlabel(_HOURS_XLABEL)

            # Sky (4-panel only): the precomputed az/el track (halo-stroked for
            # contrast, never-visible warning suppressed), the live sun-tint, and
            # Sun/Moon/live-sat glyphs — every JVM quantity from one observer_snapshot
            # station-frame build. A small corner legend identifies the glyphs (the
            # polar bounding-box corners sit outside the disk); no panel title — the
            # figure suptitle carries the live readout for the whole dashboard.
            if ax_sky is not None and station is not None:
                ax_sky.clear()
                sat_ae, sun_ae, moon_ae = observer_snapshot(station, now, state_now)
                _draw_sky_track(
                    cast("PolarAxes", ax_sky),
                    buffer,
                    station,
                    azel=engine.azel,
                ...
                # Override the primitive's fixed light-blue disk with the live tint.
                ax_sky.set_facecolor(_twilight_facecolor(sun_ae.elevation_deg))
                _plot_sky_glyph(ax_sky, sun_ae, _SUN_MARKER)
                _plot_sky_glyph(ax_sky, moon_ae, _MOON_MARKER)
                _plot_sky_glyph(ax_sky, sat_ae, _LIVE_SKY_MARKER)
                ax_sky.legend(
                    handles=[
            ...
            # Whole-figure live readout (replaces the per-panel titles): sub-satellite
            # position, altitude, both speeds, and the UTC clock.
            fig.suptitle(
        ...
        # Clear-and-redraw with blit=False: the return value is unused, but the
        # FuncAnimation stub types ``func`` as returning the changed artists.
        return []

    anim = FuncAnimation(
        fig, update, interval=int(refresh_s * 1000.0), cache_frame_data=False
    )
    logger.info(
