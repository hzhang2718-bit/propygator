...
``--emit-fixture`` prints the Chunk 4 pinned-test literals (t0 PV + hourly
day-1 truth positions) and exits -- JVM-free; the source of
``tests/propagation/test_real_world_lageos.py``'s in-file fixture.
"""
...
def emit_fixture(eph) -> None:
    """Print the Chunk 4 pinned-test literals: t0 PV + hourly day-1 truth.

    Every 30th sample of the 120 s truth grid over day 1 (25 samples, offsets
    k*3600 s -- exactly the ``output_step=3600`` grid the pinned test
    propagates on). ``repr`` floats round-trip exactly.
    """
    if eph.velocities_ms is None:
        raise SystemExit("--emit-fixture needs V-records for the t0 velocity")
    def row(a) -> str:
        return repr(tuple(float(v) for v in a))  # plain-float repr, exact round-trip

    idx = range(0, 721, 30)
    t0 = eph.epochs[0]
    print("# --- pinned fixture generated by run_lageos.py --emit-fixture ---")
    print(f"# source: {eph.sat_id} ({eph.coordinate_system}), see lageos/README.md")
    print(f'T0_ISO = "{t0.to_iso()}"  # {t0.scale.value}, ITRF')
    print(f"T0_POS_M = {row(eph.positions_m[0])}")
    print(f"T0_VEL_MS = {row(eph.velocities_ms[0])}")
    print("STEP_S = 3600.0  # truth sample spacing below (day 1, hourly)")
    print("TRUTH_POS_M = (")
    for i in idx:
...
def main() -> None:
    ...
    if "--emit-fixture" in sys.argv:
        ...
        if not files:
            raise SystemExit(f"no SP3 file under {DATA_DIR} (see README.md)")
        emit_fixture(parse_sp3(files[0]))
        return
