...
**Sanctioned exception — `tle/fitter.py` → `propagation/` (call-time only).** `fit_tle` / `fit_tle_detailed`'s `State` path is contract-bound (features §1.2) to build its reference trajectory via `propagate_numerical` with the caller's `ForceModelConfig` / `SpacecraftConfig` — a `tle → propagation` edge this rule predates. It exists **only at call time**: the propagation imports live inside the function body (the `export_all` idiom below) and the config types appear in annotations under `TYPE_CHECKING`, so `tle/`'s static module graph stays `core`-only and importing `propygator.tle` alone still pulls nothing from `propagation/`.
...
- The Feature 1.5 pure-Python consumers of the `Pass` value type: `passes_to_dataframe`, `export_passes_csv` / `export_passes_ics`, and `plot_pass_timeline` (no JVM; the JVM-touching producers `find_passes` / `compute_magnitude` / `plot_sky_chart` are listed below).
- `FitResult` construction and validation (Feature 1.2's pure-Python value type; the fit verbs that produce it are listed below).

JVM startup is reserved for: any `to_orekit()` call, `propagate_numerical`, `propagate_tle`, `fit_tle` / `fit_tle_detailed`, `TLE.from_state_unfitted` (it lowers the state to Orekit's TLE formatter), `current_state`, `current_ground_position`, `look_angles` / `look_angles_track` / `sun_look_angles` / `moon_look_angles` / `observer_snapshot` (they build an Orekit `TopocentricFrame`), `find_passes`, `compute_magnitude` (`tracking.visibility`; it fetches the Sun), `plot_sky_chart` (it recomputes each pass's arc), and `Trajectory.at()` (which builds the cached `Ephemeris`). `TLE.from_norad_id` / `fetch_tle` are **network-touching** (not JVM-touching) but likewise sit outside the safe-before-init surface. Code review and CI tests guard against accidental Orekit imports leaking into the "safe before init" surface.
...
**All five features are built:** 1.1–1.4 released as `v0.1.0`–`v0.4.0`
(including the 1.1 addenda), followed by the five general upgrades batched as
`v0.5.0`, **1.5 Ground passes** as `v0.6.0` (features §1.5 + its Outcome note),
and finally **1.2 TLE fitter** (features §1.2 + its Outcome note) — the last
v1 target verb.
...
- **`fit_tle` contract** — accepts `State | Trajectory`; `force_models` used only in the State case; emits warning if supplied with a Trajectory. Full contract drafted 2026-07-07 (features §1.2): `spacecraft` follows the same State-only rule; adds `fit_bstar`, output-identity `norad_id`/`name`, and the committed `progress` (indeterminate mode); non-convergence raises the new `TLEFitError` (a `PropygatorError` sibling of `PropagationError`). **Amended 2026-07-10 at the feature's Checkpoint A** (probe evidence in `experiments/tle-fitting/`): the sibling `fit_tle_detailed` returns the `FitResult` diagnostics (see the resolved deferral below), and the default seed is the fixed-point refinement (`FixedPointTleGenerationAlgorithm`) of the `from_state_unfitted` template.
...
- **`FitResult` return type for `fit_tle` — RESOLVED (1.2 Checkpoint A, 2026-07-10).** The Chunk-0 feasibility probe (`experiments/tle-fitting/`) showed per-measurement residuals + iteration/RMS data fall out of Orekit's `BatchLSEstimator` essentially free (the `BatchLSObserver` receives an `EstimationsProvider` + the LS `Evaluation` every iteration), so the maintainer exercised this revisit within the 1.2 branch: `fit_tle_detailed()` (identical parameters; the shared engine) returns the frozen `FitResult(tle, iterations, evaluations, rms_m, residuals_m, measurement_epochs)`; `fit_tle` stays `-> TLE` as the thin `.tle` wrapper. The sketch's `converged` flag was dropped — non-convergence raises `TLEFitError` with no partial result, so the flag would be a constant `True`. Binding contract: features §1.2 "`FitResult` and `fit_tle_detailed`".
