...
>
> **Amended 2026-07-18 / 2026-07-19 (real-world-validation Chunks 6–7, maintainer-approved, released together).** `FitResult` gains the estimator's **raw** physical parameter covariance, its parameter labels, and the a-posteriori variance factor σ₀, plus a derived `sigmas` property (Chunk 6, 2026-07-18; see "Covariance and σ₀" below), and the residual diagnostics — per-measurement velocity residual norms + the signed RIC position-residual decomposition (Chunk 7, 2026-07-19; see "Residual diagnostics" below) — additive fields only, no verb signature changes. The correlation-matrix property elected 2026-07-17 was **dropped at the chunk-time probe**: its motivating corr(B\*, n) read measures ≈ −0.97 on weak *and* strong arcs (B\*/mean-motion near-collinearity is structural to the TLE fit, not a pathology flag), and the whole-matrix conditioning read actively misleads (the healthy long arc shows the *worse* correlation conditioning). The covariance basis is **Cartesian, not mean elements** — the only basis Orekit 13.1.x's `TLEPropagatorBuilder` offers (probe-verified: no orbit-type option exists).
...
class FitResult:
    ...
    velocity_residuals_ms: np.ndarray      # per-measurement velocity residual norms, m/s, shape (N,) (2026-07-19)
    residuals_ric_m: np.ndarray            # signed position residuals, RIC axes [radial, along, cross], meters, shape (N, 3) (2026-07-19)
```
...
### Residual diagnostics (added 2026-07-19, real-world-validation Chunk 7)

Elected 2026-07-17 from the Chunk 6 scoping discussion: `residuals_m` keeps only norms, which destroys the most diagnostic information in the residual set — *structure*. The signed components were already computed and discarded in the observer loop; only the carrying surface is new.

- **Mechanism.** The `BatchLSObserver` proxy captures the full observed and estimated PV 6-vectors at every evaluation (last callback = the accepted final estimate, the same mechanism the norms already rode); the velocity norms and the RIC decomposition are derived once, after convergence, in NumPy.
- **Axis convention.** Per measurement, from the **observed** PV (already TEME, so no Earth-rotation correction): radial along the observed position unit vector; cross-track along the orbit normal `r × v` (unit); along-track completing the right-handed triad (`cross × radial` — near the velocity direction for near-circular orbits). Columns of `residuals_ric_m` are `[radial, along-track, cross-track]`; the projection is orthonormal, so **row norms reproduce `residuals_m`** (pinned to machine precision). Sign convention: **observed − estimated** — a positive along-track entry means the reference runs ahead of the fitted TLE.
- **The read** (the analysis the study's Leg 3 did by hand, now one attribute away): *periodic* (once-per-rev) structure is SGP4's short-period representation error — irreducible, the fit is as good as SGP4 gets; a *secular* along-track ramp is a dynamics mismatch — revisit `fit_bstar` / the fitting span. Measured on the pinned real GNV1B day: RIC RMS 116.5 / 594.4 / 162.1 m — the ~627 m post-fit RMS is dominated by along-track structure (the SGP4 lossiness class made anatomical). The dominance is a full-arc property, **not** pinned on short arcs — on the ~1-rev arc it inverts (cross-track leads), a fit-absorption artifact.
- **Velocity residual norms** (`velocity_residuals_ms`, m/s) complete the PV picture — the fit weighs velocity at 1 mm/s but previously reported nothing about it (measured 0.12–1.26 m/s on the pinned day).
- **RIC-only, by decision (2026-07-19).** The raw signed TEME residual vectors are deliberately **not** stored: the satellite sweeps through the TEME axes once per rev, so raw components mix orbit-frequency rotation into any structure — no diagnostic read exists there — and they are one call away (propagate the fitted TLE at `measurement_epochs` via `propagate_tle`, subtract the reference's TEME positions — the `rms_m` propagate-back doctrine). Velocity residuals ship as norms only; an RIC velocity decomposition is an additive follow-on if a read for it ever materializes.
- Both fields follow the `residuals_m` array-backed value-type invariants (defensive copy, read-only contents, value-based `__eq__`/`__hash__`, validating `__post_init__` — shapes aligned with `measurement_epochs`, velocity norms ≥ 0, RIC entries signed) and are always populated on a converged fit — unlike `covariance`, there is no `None` case.
...
- **Residual diagnostics (2026-07-19).** Pure rows: validation (shape/dtype/finiteness/alignment, signed RIC entries accepted, negative velocity norms rejected) / read-only / equality-hash, all pre-JVM. Live pins: shapes aligned with the measurement grid; RIC row norms reproduce `residuals_m` (machine precision, self-fit and real data); the along-track-dominance structure pin on the pinned GNV1B day (measured 5.1× radial / 3.7× cross-track, pinned at a generous 1.5×) — full day only, the short arc measurably inverts.
