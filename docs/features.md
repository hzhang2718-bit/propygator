...
> **Status: DRAFTED (design settled; buffer magnitudes are tunable placeholders).** Two layers: (a) the cheap **real-time primitives** (`current_state` / `current_ground_position`) sketched in architecture §7, and (b) a **live, buffered dashboard** — the substance of this feature. The signature, panel set, and behaviour below are settled and binding; only the **buffer magnitudes** (`output_step` / `half_window_s` / `refresh_s`) are placeholder defaults, deliberately left to be dialled in against measured look/feel and compute cost during the build (they are tunable parameters, not contract). 1.4 also first builds the sky-view infrastructure (the `look_angles` primitive + sky-track plot) — before 1.5, so 1.5's topocentric foundation is pulled forward (architecture §12).
...
**Signature** (buffer magnitudes are tunable placeholders, not contract — see status above):
...
def live_track(
    ...
    *,
    output_step: float = 10.0,              # PLACEHOLDER — buffer sampling cadence, seconds
    half_window_s: float = 2700.0,          # PLACEHOLDER — buffer half-span: covers [now − half_window_s, now + half_window_s]
    refresh_s: float = 1.0,                 # PLACEHOLDER — wall-clock redraw interval, seconds
...
- **No never-visible warning.** `plot_sky_track`'s one-time *never-visible* `warnings.warn` (above) was tailored to a **static** multi-hour sky view; in the live dashboard the satellite is below the station horizon most of the time and the panel redraws every frame, so a transient empty sky disk is the normal, expected state — not a condition to warn about (and a per-frame warn under clear-and-redraw would spam). The dashboard therefore **suppresses** that warning through the same `_draw_sky_track` styling/behaviour seam; the blank disk stands on its own. The standalone `plot_sky_track` verb keeps the warn-once unchanged (a static view genuinely wants it).
...
- **Ground-track panel** — live sub-satellite **lat/lon (+ alt) in the title**, a distinct current-position marker, and a **UTC clock** (formatted through a tz-ready seam — UTC now, additive `tz=` later, no core changes).
...
- **Dependency rule** — `tracking/live.py` lazily imports `plotting/` *and* matplotlib in-body (the `export_all` precedent); the buffer/refresh engine is headless-testable; the animation loop is not snapshot-tested. **`live_track` is re-exported at the top level** (zero invariant cost — the in-body imports keep `import propygator` matplotlib-free and JVM-free).

### Still open / deferred for 1.4

- **Buffer magnitudes** — concrete `half_window_s` / `output_step` / `refresh_s` defaults, and whether to surface the buffer re-propagation trigger as a knob; dialled in against look/feel + measured compute during the build.
